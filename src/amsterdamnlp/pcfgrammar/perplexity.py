...
class Perplexity(evaluate.Measurement):
    ...
    def _compute(
        ...
        for start_index in logging.tqdm(range(0, len(encoded_texts), batch_size)):
            ...
            #print("ATTN", attn_mask.shape, attn_mask)
            ...
            token_ppl = torch.exp(
                (loss_fct(shift_logits.transpose(1,2), shift_labels) * shift_attention_mask_batch) / shift_attention_mask_batch
            )
            token_ppl = [t[~t.isnan()].tolist() for t in token_ppl]
            token_ppls.append(token_ppl)

        return {"token_perplexities": token_ppls, "perplexities": ppls, "mean_perplexity": np.mean(ppls)}
